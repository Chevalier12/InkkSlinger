...
static int PrintUsage(int exitCode = 1)
{
    ...
    Console.Error.WriteLine("  inkkoops live --attach --command <ping|get-host-info|get-property|assert-property|assert-exists|assert-not-exists|move-pointer|move-pointer-target|hover|click|invoke|drag|double-click-target|right-click-target|leave-target|pointer-down|pointer-up|pointer-down-target|pointer-up-target|key-down|key-up|text-input|set-clipboard-text|maximize-window|resize-window|wait-frames|wait-for-element|wait-for-visible|wait-for-enabled|wait-for-in-viewport|wait-for-interactive|wait-for-idle|wheel|scroll-to|scroll-by|scroll-into-view|get-telemetry|get-target-diagnostics|screenshot|take-screenshot|capture-frame|dump-telemetry|move-pointer-path|drag-path-target|assert-automation-event|run-scenario|probe-during-drag|probe-scrollbar-thumb-drag|probe-action|assert-nonblank|diff-telemetry> [--scenario <json-file>] [--scope <name>] [--owner <name>] [--target <name>] [--property <name>] [--expected <value>] [--key-name <name>] [--text <text>] [--event-type <type>] [--button <left|right|middle|xbutton1|xbutton2>] [--waypoints <json>] [--width <px>] [--height <px>] [--x <value>] [--y <value>] [--anchor <center|top-left|top-right|bottom-left|bottom-right|offset>] [--offset-x <value>] [--offset-y <value>] [--frames <count>] [--samples <count>] [--axis <vertical|horizontal>] [--from <top|bottom|left|right|0..1>] [--to <top|bottom|left|right|0..1>] [--travel-frames <count>] [--step-distance <value>] [--easing <linear|ease-in-out>] [--dwell-frames <count>] [--delta <value>] [--delta-x <value>] [--delta-y <value>] [--horizontal <percent>] [--vertical <percent>] [--padding <value>] [--min-bright-pixels <count>] [--min-average-luma <value>] [--artifact <name>] [--compact] [--counters <names>] [--pipe <name>] [--timeout <ms>] [--artifacts <path>]");
...
static InkkOopsPipeRequest? BuildAttachRequest(Dictionary<string, string> options, int timeoutMilliseconds)
{
    ...
    var requestKind = commandText.Trim().ToLowerInvariant() switch
    {
        ...
        "move-pointer-target" => InkkOopsPipeRequestKinds.MovePointer,
        ...
        "probe-scrollbar-thumb-drag" => InkkOopsPipeRequestKinds.ProbeScrollbarThumbDrag,
        "probe-scrollbar-drag" => InkkOopsPipeRequestKinds.ProbeScrollbarThumbDrag,
    ...
    return new InkkOopsPipeRequest
    {
        ...
        Axis = options.TryGetValue("axis", out var axis) ? axis : string.Empty,
        From = options.TryGetValue("from", out var from) ? from : string.Empty,
        To = options.TryGetValue("to", out var to) ? to : string.Empty,
        SampleCount = options.TryGetValue("samples", out var samplesText) && int.TryParse(samplesText, NumberStyles.Integer, CultureInfo.InvariantCulture, out var samples)
            ? samples
            : 0,
