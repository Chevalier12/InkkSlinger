...
public class PasswordBox : Control, IRenderDirtyBoundsHintProvider
{
    ...
    public static readonly RoutedEvent PasswordChangedEvent =
        new(nameof(PasswordChanged), RoutingStrategy.Bubble);
        ...
        DependencyProperty.Register(
            ...
            new FrameworkPropertyMetadata(
                ...
                propertyChangedCallback: static (dependencyObject, args) =>
                {
                    ...
                    {
                        passwordBox.OnPasswordPropertyChanged(args.NewValue as string ?? string.Empty);
                    }
                },
                coerceValueCallback: static (dependencyObject, value) =>
                {
                    var text = value as string ?? string.Empty;
                    if (dependencyObject is PasswordBox passwordBox && passwordBox.MaxLength > 0 && text.Length > passwordBox.MaxLength)
                    {
                        return text[..passwordBox.MaxLength];
                    }
        ...
        DependencyProperty.Register(
            ...
            new FrameworkPropertyMetadata(
                ...
                propertyChangedCallback: static (dependencyObject, _) =>
                {
                    if (dependencyObject is PasswordBox passwordBox)
                    {
                        passwordBox.CoerceTextToMaxLength();
                    }
                },
    ...
    public static readonly DependencyProperty AllowClipboardCopyProperty =
        DependencyProperty.Register(
            nameof(AllowClipboardCopy),
            typeof(bool),
            typeof(PasswordBox),
    ...
    public PasswordBox()
    {
        ...
        _editor.SetText(Password, preserveCaret: false);
    }

    public event EventHandler<RoutedSimpleEventArgs> PasswordChanged
    {
    ...
    public string Password
    {
        get
        {
            ...
            return GetValue<string>(PasswordProperty) ?? string.Empty;
        }
    ...
    {
        get => GetValue<string>(PasswordCharProperty) ?? "\u2022";
        set => SetValue(PasswordCharProperty, value);
    }

    public bool RevealPassword
    {
        get => GetValue<bool>(RevealPasswordProperty);
        set => SetValue(RevealPasswordProperty, value);
    }

    public bool AllowClipboardCopy
    {
        get => GetValue<bool>(AllowClipboardCopyProperty);
    ...
    public PasswordBoxPerformanceSnapshot GetPerformanceSnapshot()
    {
        var syncMilliseconds = TicksToMilliseconds(_perfTextSyncTicks);
        return new PasswordBoxPerformanceSnapshot(
            _perfCommitCount,
    ...
    {
        return DefaultPasswordBoxStyle.Value;
    }
    ...
    private void OnPasswordPropertyChanged(string text)
    {
    ...
    {
        RaiseRoutedEvent(PasswordChangedEvent, new RoutedSimpleEventArgs(PasswordChangedEvent));
    }
    ...
    private char ResolveMaskCharacter()
    {
        var value = PasswordChar;
    ...
    private void CommitTextSyncNow()
    {
        ...
        try
        {
            SetValue(PasswordProperty, text);
        }
        finally
        {
    ...
    private static Style BuildDefaultPasswordBoxStyle()
    {
    ...
    internal bool HandleKeyDownFromInput(Keys key, ModifierKeys modifiers)
    {
        ...
        switch (key)
        {
            ...
            case Keys.C:
                ...
                {
                    handled = true;
                    if (AllowClipboardCopy && _editor.HasSelection)
                    {
            ...
            case Keys.X:
                ...
                {
                    handled = true;
                    if (!IsReadOnly && AllowClipboardCopy && _editor.HasSelection)
                    {
                        TextClipboard.SetText(_editor.GetSelectedText());
                        var editStart = Stopwatch.GetTimestamp();
                        changed = _editor.DeleteSelectionIfPresent();
            ...
            case Keys.V:
                ...
                {
                    handled = true;
                    if (TextClipboard.TryGetText(out var pastedText) && !string.IsNullOrEmpty(pastedText))
                    {
                        var editStart = Stopwatch.GetTimestamp();
                        changed = InsertTextIntoEditor(NormalizeLineEndings(pastedText).Replace("\r", string.Empty).Replace("\n", string.Empty));
                        editTicks = Stopwatch.GetTimestamp() - editStart;
                    }
                }
