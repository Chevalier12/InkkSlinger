...
public class DataGrid : ItemsControl
{
    ...
    private Type? _autoGeneratedItemType;
    ...
    protected override void OnItemsIncrementalChanged(NotifyCollectionChangedEventArgs e)
    {
        ...
        // For auto-generated columns, keep incremental path when item type shape is stable.
        if (_columns.Count == 0)
        {
            var newItemType = ResolveFirstNonNullItemType(e.NewItems);
            ...
            if (_autoGeneratedItemType != null &&
                newItemType != null &&
                !ReferenceEquals(_autoGeneratedItemType, newItemType))
            {
                OnItemsChanged();
                return;
            }

            _autoGeneratedItemType ??= newItemType;
        }

        var startIndex = e.NewStartingIndex;
        if (startIndex < 0)
        {
            startIndex = Math.Max(0, ItemContainers.Count - e.NewItems.Count);
        }

        startIndex = Math.Clamp(startIndex, 0, Math.Max(0, ItemContainers.Count - 1));
        SyncRowsHost(startIndex);
    }
    ...
    internal object? GetValueForCell(object? item, DataGridColumn column)
    {
        ...
        if (!_valueReaderCache.TryGetValue(path, out var reader))
        {
            reader = BuildValueReader(path, item.GetType());
            _valueReaderCache[path] = reader;
        }
    ...
    private void RefreshAutoColumnsIfNeeded()
    {
        ...
        else if (ItemContainers.Count > 0)
        {
            ...
            if (sample != null)
            {
                _autoGeneratedItemType = sample.GetType();
        ...
        SyncResolvedColumnSortDirectionsFromView();
    ...
    private static Type? ResolveFirstNonNullItemType(System.Collections.IList items)
    {
        for (var i = 0; i < items.Count; i++)
        {
            if (items[i] != null)
            {
                return items[i]!.GetType();
            }
        }
        ...
        for (var i = 0; i < segments.Length; i++)
        {
            ...
            if (!currentType.IsValueType)
            {
                current = Expression.Condition(
                    Expression.ReferenceEqual(current, Expression.Constant(null, currentType)),
    ...
    private void SyncResolvedColumnSortDirectionsFromView()
    {
        var view = ItemsSourceView;
        for (var i = 0; i < _resolvedColumns.Count; i++)
        {
            _resolvedColumns[i].SortDirection = ResolveColumnSortDirection(view, _resolvedColumns[i].BindingPath);
        }
    }
    ...
    private static DataGridSortDirection ResolveColumnSortDirection(ICollectionView? view, string? bindingPath)
    {
        ...
        for (var i = view.SortDescriptions.Count - 1; i >= 0; i--)
        {
            ...
            if (!string.Equals(description.PropertyName ?? string.Empty, normalizedBindingPath, StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
    ...
    private void SyncRowsHost(int startIndex = 0)
    {
    ...
    private void OnColumnHeaderClick(object? sender, RoutedSimpleEventArgs e)
    {
        ...
        var previousSortDirection = column.SortDirection;
        ClearOtherColumnSortDirections(index);
        ...
        if (previousSortDirection != column.SortDirection)
        {
    ...
    private void ClearOtherColumnSortDirections(int activeIndex)
    {
        for (var i = 0; i < _resolvedColumns.Count; i++)
        {
            if (i == activeIndex)
            ...
            _resolvedColumns[i].SortDirection = DataGridSortDirection.None;
        }
    }

    private void SyncHeaderSortDirections()
    {
        var count = Math.Min(_columnHeaders.Count, _resolvedColumns.Count);
        for (var i = 0; i < count; i++)
        {
            _columnHeaders[i].SortDirection = _resolvedColumns[i].SortDirection;
        }
    }
