...
public class DataGridCollectionViewTests
{
    ...
    [Fact]
    public void DataGrid_EmptySource_DoesNotInventFallbackAutoGeneratedColumn()
    {
        var grid = new DataGrid
        {
        ...
        Assert.Empty(grid.DisplayColumnsForTesting);
    ...
    [Fact]
    public void DataGrid_HeaderPointerUpOutsideOriginalHeader_DoesNotSort()
    {
        var source = new ObservableCollection<Row>
        {
            new(2, "B"),
        ...
        var headerCenter = new Microsoft.Xna.Framework.Vector2(header.LayoutSlot.X + (header.LayoutSlot.Width / 2f), header.LayoutSlot.Y + (header.LayoutSlot.Height / 2f));
        Assert.True(grid.HandlePointerDownFromInput(header, headerCenter, ModifierKeys.None, out _));
        Assert.True(grid.HandlePointerUpFromInput(new Microsoft.Xna.Framework.Vector2(header.LayoutSlot.X - 20f, header.LayoutSlot.Y - 20f)));
