...
public sealed class ScrollViewerWheelHoverRegressionTests
{
    ...
    public void WheelScroll_WithStationaryPointer_UpdatesHoverToElementNowUnderCursor()
    {
        ...
        // Exercise the scroll+hover scenario with instrumentation capture
        using var capture = new InstrumentationCapture();
        Wheel(uiRoot, pointer, delta: -120);
        RunLayout(uiRoot, 280, 220, 16);
        var lines = capture.GetInstrumentLines();

        // Parse instrumentation: collect timing and counter data
        ...
        // Report hot methods (high call counts)
